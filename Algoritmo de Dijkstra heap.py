...
#Con una estructura heap, priority queue (cola de prioridad)

#Objetivo de esta estructura:
#optimizar la selección del vertice con la distancia más corta en cada iteración, reduciendo el número de operaciones 
...
def dijkstra(grafo, inicio, fin):
    distancia = {}
    padres = {}
    # Incluimos todos los nodos: los que son origen Y los que solo aparecen como vecinos
    todos_nodos = set(grafo.keys())
    for vecinos in grafo.values():
        for nodo, _ in vecinos:
    ...
    #Funcion PUSH : Agrega un nuevo elemento al heap, manteniendo la propiedad de orden 
    #Funcion POP : Extraemos el vertice con la distancia min 
    #Funcion ADD : Agregamos el vertice al conjunto de explorados 
    ...
    while heap :
        ...
        for adyacente, peso in grafo.get(vertice, []):
