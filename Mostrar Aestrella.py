"""
Mostrar Aestrella.py — A* animado sobre el Metro de Madrid.
...
# ── heurística (distancia en línea recta, fórmula de Haversine) ──────────────
...
# ── A* generador ──────────────────────────────────────────────────────────────
def astar_gen(grafo, inicio, fin, node_coords):
    ...
    def h(n):
        if n in node_coords and fin in node_coords:
            return distancia_haversine(node_coords[n], node_coords[fin])
        return 0.0
    ...
    while heap:
        ...
        #Un break, para terminar el bucle while cuando se llega a destino,
...
def dibujar_hud(screen, fonts, explored, dist_m, transbordos, speed, paused, done,
    ...
    screen.blit(font_big.render("A*", True, (90, 150, 255)), (16, 14))
...
def main():
    ...
    pygame.display.set_caption("A* — Metro de Madrid")
    ...
    def reset_sim(orig, dest):
        ...
        return {
            'gen'         : astar_gen(grafo, orig, dest, node_coords),
            'explored'    : 0,
            'current'     : None,
            'current_dist': 0.0,
            'final_path'  : None,
